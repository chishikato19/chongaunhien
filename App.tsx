...
function App() {
  ...
  const startRandomizer = () => {
    ...
    // 1. Initial Filtering (Absent & Pending)
    let eligiblePool = activeClass.students.filter(s => !pendingStudents.find(p => p.id === s.id) && !s.isAbsent);
    
    // 2. Cooldown Logic (Avoid repeating recent 5 picks)
    const recentHistory = activeClass.recentPickHistory || [];
    const nonRecentPool = eligiblePool.filter(s => !recentHistory.includes(s.id));
    
    // Fallback if everyone is recent (e.g. class size < 5)
    if (nonRecentPool.length > 0) eligiblePool = nonRecentPool;
    ...
    if (activeQuestion && activeQuestion.difficulty) {
        if (activeQuestion.difficulty === 'HARD') {
            ...
            if (goodStudents.length > 0) eligiblePool = goodStudents;
                 ...
                 const fairStudents = eligiblePool.filter(s => s.academicLevel === 'FAIR');
    ...
    const weightedStudents = eligiblePool.map(s => {
        ...
        totalWeight += weight;
        return { student: s, weight };
    });

    let randomVal = Math.random() * totalWeight;
    let pickedWinner = weightedStudents[0].student;
    
    for (const item of weightedStudents) {
        randomVal -= item.weight;
        if (randomVal <= 0) {
            pickedWinner = item.student;
            break;
        }
    }
  ...
  const resetData = () => {
    ...
    if (window.confirm('CẢNH BÁO: Bạn muốn đặt lại điểm số phiên?')) {
        const updatedClasses = classes.map(c => c.id === activeClassId ? { ...c, students: c.students.map(s => ({ ...s, score: 0, lastPickedDate: null })), recentPickHistory: [] } : c);
