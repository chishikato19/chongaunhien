...
import { Play, Settings as SettingsIcon, Mic, Maximize, Minimize, Clock, RotateCcw, LogOut, HelpCircle, BookOpen, Hand, Youtube, X, PartyPopper, ShoppingBag, Info, AlertCircle } from 'lucide-react';
...
function App() {
  ...
  const [showChangelog, setShowChangelog] = useState(true); // Show on start
  ...
  const onQuestionCorrect = () => {
      ...
      if (winner && !isGroupSpin && activeClass) {
           ...
           // Knowledge King check here too, although score hasn't changed yet, badges might have
           handleUpdateClasses(classes.map(c => c.id === activeClass.id ? { ...activeClass, students: updatedStudents } : c));
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-20 shadow-sm">
          <div className="container mx-auto px-4 py-3 flex justify-between items-center">
              <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2"><div className="bg-indigo-600 text-white p-1.5 rounded-lg"><Play size={20} fill="currentColor"/></div><span className="font-bold text-lg tracking-tight text-gray-800">ClassRandomizer</span></div>
                  <div className="flex items-center gap-1 md:gap-2">
                      ...
                      {currentView === 'SESSION' && <><button onClick={resetData} className="p-2 bg-gray-100 rounded-lg"><RotateCcw size={16}/></button><button onClick={() => setShowEndConfirm(true)} className="p-2 bg-red-50 text-red-600 rounded-lg"><LogOut size={16}/></button></>}
                  </div>
              </div>
          </div>
      </header>
      ...
      {showTimerModal && (
          <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4">
              <div className={`${isTimerFullScreen ? 'fixed inset-0 w-full h-full max-w-none rounded-none bg-indigo-900 text-white flex flex-col justify-center' : 'bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl relative'}`}>
                   ...
                   <button onClick={() => setIsTimerFullScreen(!isTimerFullScreen)} className="absolute top-4 left-4 p-2 hover:bg-white/20 rounded-full">{isTimerFullScreen ? <Minimize size={24}/> : <Maximize size={24}/>}</button>
      ...
      {/* Footer Version Info */}
      <button onClick={() => setShowChangelog(true)} className="fixed bottom-2 right-2 text-[10px] text-gray-400 hover:text-gray-600 bg-white/50 px-2 py-1 rounded">V2.1</button>
    </div>
  );
}
