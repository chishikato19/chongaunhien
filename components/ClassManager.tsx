...
import { ClassGroup, Student, Gender, Settings } from '../types';
...
const ClassManager: React.FC<ClassManagerProps> = ({ classes, activeClassId, onUpdateClasses, onSetActive, settings }) => {
  ...
  const addStudent = (name: string, gender: Gender = 'M') => {
    if (!activeClass) return;
    // Use common avatars from settings, filtering out ones used in this class if possible
    const usedAvatars = activeClass.students.map(s => s.avatar);
    // Helper to get unique from available pool
  ...
  const processImport = () => {
    ...
    const newStudents: Student[] = lines.map(line => {
      ...
      // Get unique avatar logic local to this loop
