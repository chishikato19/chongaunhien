...
const ShopModal: React.FC<ShopModalProps> = ({ students, settings, onClose, onPurchase }) => {
    ...
    const getPrice = (avatar: string) => {
        const tier = settings.avatarTiers?.[avatar] || 1;
        if (tier === 3) return settings.priceTiers.tier3;
        if (tier === 2) return settings.priceTiers.tier2;
        return settings.priceTiers.tier1;
    };

    const handleBuy = (avatar: string) => {
        if (!activeStudent) return;
        const cost = getPrice(avatar);
                            ...
                            <>
                                ...
                                <div className="p-6 overflow-y-auto grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-4">
                                    {settings.specialAvatars.map(avatar => {
                                        ...
                                        const cost = getPrice(avatar);
                                        const canAfford = (activeStudent.balance || 0) >= cost;
                                        const tier = settings.avatarTiers?.[avatar] || 1;
                                        
                                        let tierColor = 'border-gray-100';
                                        if (tier === 2) tierColor = 'border-blue-200 bg-blue-50/30';
                                        if (tier === 3) tierColor = 'border-yellow-200 bg-yellow-50/30';

                                        return (
                                            <div key={avatar} className={`relative group p-4 rounded-xl border-2 flex flex-col items-center justify-center transition-all 
                                                ${isOwned ? 'bg-green-50 border-green-200' : `${tierColor} hover:shadow-md`}
                                            `}>
                                                ...
                                                {isOwned ? (
                                                    ...
                                                    >
                                                        {canAfford ? 'Mua' : 'Thiếu'}
                                                        <span className="bg-black/20 px-1 rounded ml-1">{cost}</span>
                                                    </button>
                                                )}
